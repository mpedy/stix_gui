...
require(["domReady!", "stix2viz/stix2viz/stix2viz"], function (document, stix2viz) {
     ...
     * ******************************************************/
    function vizStixWrapper(content, customConfig, toggle, elemsIDs) {
        /*let uploader = document.getElementById('uploader');
        let canvasContainer = document.getElementById('canvas-container');
        let canvas = document.getElementById('canvas');
        */
        let uploader = document.getElementById(elemsIDs[0]);
        let canvasContainer = document.getElementById(elemsIDs[1]);
        let canvas = document.getElementById(elemsIDs[2]);
        ...
        customConfig.iconDir = "/stix/cti-stix-visualization/stix2viz/stix2viz/icons";
        ...
        try
        {
            ...
            } else
            {
                ...
                view.graph.on("beforeDrawing", function(ctx){
                    /*let relid = Array.from(stixIdToObject.keys()).filter(a => a.indexOf("relationship")>=0)[0].replace("relationship--","");
                    let others = Array.from(stixIdToObject.keys()).filter(a => a.indexOf("relationship")<0);
                    let aa = view.graph.body.edges["relationship--"+relid];
                    let a1 = view.graph.body.nodes[others[0]]
                    let a2 = view.graph.body.nodes[others[1]]
                    //aa.options.backgroundColor="#ff3456";
                    console.log(aa.options);
                    aa.options.color="red";
                    aa.updateEdgeType();
                    ctx.strokeStyle = "red";
                    ctx.lineWidth = 2
                    ctx.beginPath();
                    ...
                    /*view.graph.body.edges["relationship--"+relid].options.color = "#ff0000";
                    view.graph.body.edges["relationship--"+relid].options.backgroundColor = "red";
                    */
                });
                view.on(
                        "click",
    ...
    window.vizStixWrapper = vizStixWrapper;
...
//      uploader.classList.toggle("hidden");
        document.getElementById("canvas-container").classList.toggle("hidden"); // Necessario per il riaggiornamento della pagina web: mi cambia il DOM e quindi anche gli elementi non sono più gli stessi
