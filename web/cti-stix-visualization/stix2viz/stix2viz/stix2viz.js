 ...
 */
function makeEdgeObject(sourceRef, targetRef, label, stixId=null, stixObj=null)
{
    let edge = {
        from: sourceRef,
        to: targetRef,
    ...
    if (stixObj.get("source_timeref"))
        edge.source_timeref = stixObj.get("source_timeref");
    
    if(stixObj.get("target_timeref"))
        edge.target_timeref = stixObj.get("target_timeref");
    return edge;
}
...
function edgeForRelationship(stixRel, stixIdToObject)
{
    ...
    if (stixIdToObject.has(sourceRef) && stixIdToObject.has(targetRef))
    {
        ...
        if (isStixIdValidForNode(sourceRef) && isStixIdValidForNode(targetRef))
            edge = makeEdgeObject(
                sourceRef, targetRef, relType, stixRel.get("id"), stixRel
            );
    }
    else
        console.warn(
